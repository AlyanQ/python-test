...
public:
    ...
    void __fake_save_on_db(std::tuple<std::string, int> order)
    {
        ...
        // Following line is removed since multiple threads can output to console at the same time
        // and cause output to be jumbled
        this->__log("Order [" + id + "] " + std::to_string(number) + " was successfully prosecuted.");


        // Sleep for between 0-767 milliseconds to simulate processing time
    ...
    void process_orders()
    {
        ...
        // Create a point-to-member function pointer to the __fake_save_on_db function
        // func now points to the __fake_save_on_db function
        void (OrdersManager::*func)(std::tuple<std::string, int>);
        func = &OrdersManager::__fake_save_on_db;
        ...
        for (std::tuple<std::string, int> order : this->__orders)
        {
            ...
            threads.push_back(
                // Create a new thread with the __fake_save_on_db function and pass in the order
                // Here, func is used to call the __fake_save_on_db function
                std::thread(func, this, order)
            );

            // Increment the number of orders processed
            this->__orders_processed += 1;

            // Log total orders executed every 0.1 seconds
            if (std::time(0) > this->__last_printed_log)
            {
                // Add 0.1 seconds to last printed log variable
                this->__last_printed_log = std::time(0) + 0.1;
        ...
        // This makes sure all threads are finished before the program exits
        for (auto &th : threads)
        {
            th.join();
        }
